...
def select_batch_upload(page, debug_dir: Path) -> None:
    ...
    if click_first_available_anywhere(page, selectors, timeout=15000):
        if wait_upload_area_ready(page, timeout_ms=15000):
            return

    # 兜底：用 JS 查找包含“批量上传”的 label/span/div 并点击。
    for target in page_and_frames(page):
        try:
            ...
            """)
            if clicked and wait_upload_area_ready(page, timeout_ms=15000):
                return
        except Exception:
            continue
