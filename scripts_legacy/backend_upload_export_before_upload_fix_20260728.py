...
def upload_file(page, file_path: Path) -> None:
    # 直接设置网页上传控件，不依赖 Windows 文件选择窗口，更稳定。
    ...
    with page.expect_file_chooser(timeout=15000) as chooser_info:
        ...
        if not clicked:
            raise RuntimeError("找不到上传文件按钮，请提供页面元素信息或让我检查页面后调整选择器。")
...
def main() -> int:
    ...
    with sync_playwright() as playwright:
        ...
        click_first_available(page, [("text", "批量上传"), ("text", "上传文件")], timeout=15000)

        for index, file_path in enumerate(files, start=1):
            log(f"[{index}/{len(files)}] 准备上传：{file_path.name}", args.log_file)
            click_first_available(page, [("role_button", "清空"), ("text", "清空")], timeout=3000)
            upload_file(page, file_path)

            started = click_first_available(
                page,
            ...
            exported = click_first_available(
                page,
