...
def wait_upload_area_ready(page, timeout_ms: int = 15000) -> bool:
    ...
    while time.time() < deadline:
        for target in page_and_frames(page):
            try:
                ...
                pass
            for text in ["点击上传 Excel 文件", "上传文件", "下载模板"]:
                try:
                    target.get_by_text(text, exact=False).first.wait_for(state="visible", timeout=800)
                    return True
                except Exception:
                    pass
        time.sleep(0.3)
...
def select_batch_upload(page, debug_dir: Path) -> None:
    # 页面必须先选中“批量上传”，才会出现上传文件区域。
    # 先用 JS 快速点击，避免 Playwright role/text 定位逐个等待导致变慢。
    for target in page_and_frames(page):
        try:
            clicked = target.evaluate("""
                () => {
                    const nodes = Array.from(document.querySelectorAll('label, span, div'));
                    const node = nodes.find(el => (el.innerText || '').trim().includes('批量上传'));
                    if (!node) return false;
                    const label = node.closest('label') || node;
                    const radio = label.querySelector('input[type=radio]') || node.querySelector?.('input[type=radio]');
                    if (radio && !radio.checked) radio.click();
                    label.click();
                    return true;
                }
            """)
            if clicked and wait_upload_area_ready(page, timeout_ms=2500):
                return
        except Exception:
            continue
    ...
    selectors = [
        ("role_radio", "批量上传"),
        ("label", "批量上传"),
        ("css", ".ant-radio-wrapper:has-text('批量上传')"),
        ("css", "label:has-text('批量上传')"),
        ("text", "批量上传"),
    ]
    if click_first_available_anywhere(page, selectors, timeout=2000):
        if wait_upload_area_ready(page, timeout_ms=3000):
            return

    if wait_upload_area_ready(page, timeout_ms=1000):
        return
...
def main() -> int:
    ...
    with sync_playwright() as playwright:
        ...
        for index, file_path in enumerate(files, start=1):
            ...
            click_first_available_anywhere(page, [("role_button", "清空"), ("text", "清空")], timeout=500)
            ...
            started = click_first_available_anywhere(
                page,
                [("role_button", "开始查询"), ("text", "开始查询"), ("role_button", "查询"), ("text", "查询"), ("css", "button:has-text('开始查询')"), ("css", "button:has-text('查询')")],
                timeout=5000,
            )
            if not started:
